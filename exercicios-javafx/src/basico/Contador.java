...
public class Contador extends Application{
	...
	private void atualizarLabelNumero(Label label) {
		...
		label.getStyleClass().add(contador > 0 ? "verde" : (contador < 0 ? "vermelho" : ""));
		
		/*
		 * if(contador > 0) { label.getStyleClass().add("verde"); }else if (contador <
		 * 0) { label.getStyleClass().add("vermelho"); }
		 */
	}
	...
	public void start(Stage primaryStage) throws Exception {
		...
		botaoIncremento.setOnAction(e -> {	// e= evento
			contador++;
			atualizarLabelNumero(labelNumero);
